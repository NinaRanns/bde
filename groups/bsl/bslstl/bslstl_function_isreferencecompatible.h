...
//@PURPOSE: Provide a primitive metafunction for reference compatibility.
//
//@CLASSES:
//  bslstl::Function_IsReferenceCompatible : single trait metafunction
//
//@SEE_ALSO:
//
//@DESCRIPTION: This component provides a single trait metafunction,
// 'bslstl::Function_IsReferenceCompatible', which allows generic code to
// determine whether a reference to the first template parameter type can be
// substituted for a reference to the second template parameter type with no
// loss of information.  By default, this metafunction yields 'true_type' if it
// is the same as 'TO_TYPE'; else it yields 'false_type'.  However, this
// template can be specialized to yield 'true_type for other parameters that
// have compatible references.  This metafunction is used within an 'enable_if'
// to prevent types that are reference compatible with 'bsl::function' from
// matching template parameters in 'function' constructors and assignment
// operators, preferring, instead, the non-template overloads for copy and move
...
// 'bsl::function' or a type wrapping 'bsl::function' with no additional data
// members (i.e., interface wrappers around 'bsl::function' would specialize
// this trait to derive from 'true_type'). Note that reference qualifiers on
// 'FROM_TYPE' and 'TO_TYPE' will cause instantiation to fail; the caller
// should strip reference qualifiers on 'FROM_TYPE' and 'TO_TYPE' before
// checking for reference compatibility.
//
///Usage
///-----
...
: bsl::is_same<FROM_TYPE, TO_TYPE>::type {
    ...
    // metafunction yields 'true_type' if it is the same as 'TO_TYPE'; else it
    // yields 'false_type'.  However, this template can be specialized to yield
    // 'true_type for other parameters that have compatible references.  This
    // metafunction is used within an 'enable_if' to prevent types that are
    // reference compatible with 'bsl::function' from matching template
    // parameters in 'function' constructors and assignment operators,
    ...
    // qualifiers on 'FROM_TYPE' and 'TO_TYPE' will cause instantiation to
    // fail; the caller should strip reference qualifiers on 'FROM_TYPE' and
    // 'TO_TYPE' before checking for reference compatibility.
